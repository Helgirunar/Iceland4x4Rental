@props([
    ...
    'displayTitle' => true,
    'title' => 'Iceland 4x4 Rental',
    'titleExtra' => 'Large Selection of 4x4 Rental Cars'
])
...
<form class="lg:py-12" action="/search" method="post">
    @csrf
    @if($displayTitle)
    <h1 class="mt-4 tracking-tight text-white sm:mt-5 sm:text-6xl lg:mt-6 xl:text-6xl">
        <span class="block text-4xl font-extrabold">{{$title}}</span>
        <span class="block pb-3 text-2xl md:text-3xl">{{$titleExtra}}</span>
    </h1>
    @endif
    <label for="startDate" class="text-white">{{ __('pickup')}}</label>
    ...
    <br>
    <label for="endDate" class="text-white">{{ __('return')}}</label>
    <div class="relative flex">
        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            <svg class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"></path></svg>
        </div>
    ...
    <div hidden>
        <input type="text" name="language" value="{{App::currentLocale()}}"/>
    </div>
